...
export default function FilterSidebar({ onFilterChange, currentFilters, isMobile = false, onClose }) {
  ...
  const handlePriceChange = (e) => {
    ...
    onFilterChange({ ...currentFilters, price: [0, val] });
  };
  ...
  const sidebarContent = (
    <div className={`p-6 ${isMobile ? 'h-full overflow-y-auto pb-24' : ''}`}>
      ...
      {/* Price Range */}
      <div className="mb-10">
        <div className="flex justify-between items-end mb-4 border-b border-border-color pb-2">
          <h4 className="text-white text-xs tracking-[0.2em] uppercase">Max Price</h4>
          <span className="text-gold text-xs">{formatPrice(currentFilters.price[1])}</span>
        </div>
        <div className="pt-2 px-1">
          <input 
            ...
            max="5000000" 
            step="10000"
            value={currentFilters.price[1]} 
            onChange={handlePriceChange}
            className="w-full h-1 bg-border-color rounded-lg appearance-none cursor-pointer accent-gold hover:accent-[#e8c98a]"
            style={{
              background: `linear-gradient(to right, #c9a96e ${(currentFilters.price[1] - 10000) / (5000000 - 10000) * 100}%, #222222 ${(currentFilters.price[1] - 10000) / (5000000 - 10000) * 100}%)`
            }}
          />
        </div>
      </div>
